...
public static class DomainEvents
{
    ...
    public static void Raise<T>(T args) where T : Invoicing.Domain.Core.Events.Contracts.DomainEvent
    {
        if (Container != null)
        {
            foreach (var handler in Container.ResolveAll<Invoicing.Domain.Core.Events.Contracts.IHandle<T>>())
            {
                handler.Handle(args);
            }
        }
