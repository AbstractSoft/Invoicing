...
public static class Activator
{
    public static object CreateInstance(Type type)
    {
        try
        {
            ...
            throw new InvalidOperationException("Parameterless constructor was not found for type " + type.Name);
        }
    ...
    private static T GetInstance<T>(Type[] parametersTypes, params object[] parametersValues)
    {
        ...
        var type = typeof(T);
        var ctor = type.GetConstructor(parametersTypes ?? new Type[] { });
        if (ctor != null)
    ...
    private static Type[] GetParametersTypes<T1>()
    {
        return new[] { typeof(T1) };
    }

    private static Type[] GetParametersTypes<T1, T2>()
    {
        return new[] { typeof(T1), typeof(T2) };
    }

    private static Type[] GetParametersTypes<T1, T2, T3>()
    {
        return new[] { typeof(T1), typeof(T2), typeof(T3) };
    }

    private static Type[] GetParametersTypes<T1, T2, T3, T4>()
    {
